...
class Viewer:
    ...
    def render(self):
        ...
        if index not in self.tt.pages:
            ...
            print(f"{header}\n")
            if self.mode == "ansi":
                text = page.to_ansi(colors=self.colors, border=True)
                # Path("ansi.txt").write_text(tt.to_ansi(colors=self.colors))
            else:
                text = page.to_ndjson()
            print(self.side_by_side(text, panel_str))
    ...
    @classmethod
    def side_by_side(cls, str1: str, str2: str, distance: int = 2) -> str:
        lines1 = str1.splitlines()
        lines2 = str2.splitlines()
        lines1_no_ansi = RE_ANSI_ESCAPE.sub("", str1).splitlines()
